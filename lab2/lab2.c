        ...
        {
                {
                        ...
                        start_head: 0x0,
                        part_type: 0x83,
                        end_cyl: cyl4size(0x4FFF),
                        end_sec: sec4size(0x4FFF),
                        end_head: head4size(0x4FFF),
                        abs_start_sec: 0x1,
                        sec_in_part: 0x4FFF // 10 Mbyte
                },
                {
                        boot_type: 0x00,
                        start_head: head4size(0x5000),
                        start_sec: sec4size(0x5000),
                        start_cyl: cyl4size(0x5000),
                        part_type: 0x83,
                        end_head: head4size(0xC800),
                        end_sec: sec4size(0xC800),
...
static int vramdisk_open(struct block_device *x, fmode_t mode) {
    ...
    if (dev == NULL) {
        printk(KERN_WARNING
        DISK_NAME "driver: invalid disk private_data\n");
        return -ENXIO;
    }

    atomic_inc(&dev->nr_users);
    printk(KERN_INFO
    DISK_NAME "driver: open \n");
...
int vramdisk_init(void) {
    ...
    if (device.data == NULL) {
        printk(KERN_WARNING
        DISK_NAME "driver: vmalloc failure.\n");
        return -ENOMEM;
    }
...
#define BV_LEN(bv) ((bv).bv_len)
    ...
    rq_for_each_segment(bv, req, iter)
    {
        ...
        printk(KERN_DEBUG
        DISK_NAME ": Start Sector: %llu, Sector Offset: %llu; "
    ...
    if (sector_offset != sector_cnt) {
        printk(KERN_ERR
        DISK_NAME ": bio info doesn't match with the request info");
        return -EIO;
    }
...
int device_setup(void) {
    ...
    if (major <= 0) {
        printk(KERN_WARNING
        DISK_NAME ": unable to get major number\n");
        return -EBUSY;
    }

    printk(KERN_ALERT
    "Major Number is : %d", major);
    printk(KERN_ALERT
    "CYL_SIZE : %d", CYL_SIZE);
    printk(KERN_ALERT
